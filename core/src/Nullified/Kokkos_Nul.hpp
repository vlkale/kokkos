...
class NulInternal {
 public:
  ...
  static NulSerialInternal& singleton();
...
class Nul {
 public:
  ...
  using scratch_memory_space = ScratchMemorySpace<Kokkos::Nul>;
  ...
  Impl::NulInternal* impl_internal_space_instance() const {
...
std::vector<Serial> partition_space(const Serial&, Args...) {
  ...
  std::vector<Nul> instances;
  instances.reserve(sizeof...(Args));
  std::generate_n(std::back_inserter(instances), sizeof...(Args),
                  []() { return Nul{NewInstance{}}; });
  return instances;
}

template <class T>
std::vector<Nul> partition_space(const Nul&,
...
#include <Serial/Kokkos_Nul_Parallel_Range.hpp>
#include <Serial/Kokkos_Nul_Parallel_MDRange.hpp>
#include <Serial/Kokkos_Nul_Parallel_Team.hpp>
#include <Serial/Kokkos_Nul_Task.hpp>
